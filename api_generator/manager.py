...
def generate_forms_file():
    ...
    forms = "\n\n".join(
        base_form.format(model_name=model_name) for model_name in API_GENERATOR.values()
    )
    generation = forms_structure.format(
        library_imports=library_imports, project_imports=project_imports, forms=forms
    )
...
def generate_routes_file():
    ...
    routes = "\n\n".join(
        base_routes.format(
            form_name=f"{model_name}Form", model_name=model_name, endpoint=endpoint
        )
        for endpoint, model_name in API_GENERATOR.items()
    )

    generation = routes_structure.format(
        library_imports=library_imports, project_imports=project_imports, routes=routes
    )
