...
def get_lesson():
    ...
    scenarios_data = {
        str(scenario.order_in_lesson): scenario.to_dict()
...
def calculate_user_progress(user_id, lesson_id):
    ...
    completed_scenarios = (
        ...
        .join(SubLesson)
        .filter(SubLesson.lesson_id == lesson_id)
        .count()
    )

    progress_percentage = (
        (completed_scenarios / total_scenarios) * 100 if total_scenarios else 0
    )
...
def update_user_scenario_progress(user_id, scenario_id, score):
    ...
    scenario_exists = (
        db.session.query(SubLesson.id).filter_by(id=scenario_id).first() is not None
    )
...
def analyze_speech():
    try:
        ...
        scenario = (
            SubLesson.query.filter_by(lesson_id=lesson.id)
            .order_by(SubLesson.id)
            .offset(scenario_num - 1)
            .first()
        )
        if not scenario:
...
def get_skill_progress():
    ...
    top_sentiments = sorted(
        sentiment_totals.items(), key=lambda item: item[1], reverse=True
    )[:4]

    # Convert to a dict for JSON response, scaling scores up to percentage
    top_sentiments_dict = {
        sentiment: score * 100 for sentiment, score in top_sentiments
    }
    ...
    for lesson in lessons:
        ...
        user_scenarios = (
            UserScenarioProgress.query.join(
                SubLesson, UserScenarioProgress.scenario_id == SubLesson.id
            )
            .filter(
        ...
        if user_scenarios:
            average_score = sum([scenario.score for scenario in user_scenarios]) / len(
                user_scenarios
            )
